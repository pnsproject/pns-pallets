...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		#[pallet::constant]
		type SetSubnamePrice: Get<BalanceOf<Self>>;

		#[pallet::constant]
		type BlackList: Get<alloc::vec::Vec<Self::Hash>>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn register(
			...
			let price = T::PriceOracle::registry_price(label_len, duration);
			...
			T::Registry::mint_subname(
				...
				|maybe_pre_owner| -> DispatchResult {
					...
					let target_value = price + deposit + register_fee;
					T::Currency::transfer(
						&caller,
						&official,
						target_value,
		...
		pub fn renew(
			...
			RegistrarInfos::<T>::mutate(label_node, |info| -> DispatchResult {
				...
				let price = T::PriceOracle::renew_price(label_len, expire, duration);
				T::Currency::transfer(
					&caller,
		...
		pub fn set_owner(
			...
			if let Some(info) = RegistrarInfos::<T>::get(node) {
				...
				ensure!(info.expire + T::GracePeriod::get() > now, Error::<T>::NotOwned);
			}
...
impl<T: Config> crate::traits::Registrar for Pallet<T> {
	...
	fn check_expires_useable(node: Self::Hash) -> sp_runtime::DispatchResult {
		...
		let expire = RegistrarInfos::<T>::get(node).ok_or_else(|| Error::<T>::NotExist)?.expire;
	...
	fn check_expires_renewable(node: Self::Hash) -> sp_runtime::DispatchResult {
		...
		frame_support::ensure!(now < expire + T::GracePeriod::get(), Error::<T>::NotRenewable);
